      ...
      <div class="page-content d-flex align-items-stretch"> 
        ...
        <div class="content-inner">
          ...
          <header class="page-header">
            <div class="container-fluid">
              <h2 class="no-margin-bottom">Data Backup Pendaftaran</h2>
            </div>
          </header>
          <!-- Breadcrumb-->
...
<table border="1" id="datatable1" style="width: 100%;" class="table">
                    <thead align="center">
                      <tr class="head">
                        <th>No</th>
                            <th>Nama Mahasiswa</th>     <th>No hp</th>  
    <th>Jenis Kelamin</th>
    <th>Email</th> 
    <th>Alamat</th> 
    <th>divisi1</th>
 ...
 <?php
    ...
    $Cari="SELECT * FROM backup_pendaftaran";
    $Tampil = $link->query($Cari);
    $nomer=0;
    foreach( $Tampil as $hasil) {
            $nama             = stripslashes ($hasil['nama_b']);
            $nohp             = stripslashes ($hasil['telp_b']);
            $jenis_kelamin    = stripslashes ($hasil['jenis_kelamin_b']);
            $email            = stripslashes ($hasil['email_baru_b']);
            $alamat           = stripslashes ($hasil['alamat_b']);
            $divisi1          = stripslashes ($hasil['devisi1_b']);
            $divisi2          = stripslashes ($hasil['devisi2_b']);
            $jurusan          = stripslashes ($hasil['jurusan_b']);
            $npm              = stripslashes ($hasil['angkatan_b']);
            $keterangan       = stripslashes ($hasil['keterangan_b']);
            // $Foto             = stripslashes ($hasil['photo']);
            $idline           = stripslashes ($hasil['id_line_b']);
        {
    $nomer++;
?>
  <tr>
        ...
        <td><?=$keterangan?></td>
        <td><!-- <?=$Foto?> -->
          <?="<img src='image/".$hasil['photo_b']."'style='width:70px; height:70px;'"?>
        </td>
  </tr>
   <?php  
        }
